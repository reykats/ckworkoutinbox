...
class table_workoutdb_library_equipment_media extends mysql_table {

	public function __construct() {
		parent::__construct();
		
		$this->initializeTable('workoutdb','library_equipment_media');
	}
	...
	public function insert( $p_fields ) {
		...
		$return = $this->file_upload->copy_temp_to_dir_resize_image($fields->url,'equipment');
	...
	public function delete( $p_id = null ) {
		...
		$this->load->model('file/file_upload');
		$return = $this->file_upload->delete('equipment',$filename);
		if ( $return['status'] >= 300 ) {
			$return['response']->id = null;
			return $return;
		}
