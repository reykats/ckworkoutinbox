...
class Image_moo
{
	...
	//----------------------------------------------------------------------------------------------------------
	{
		...
		$CI = get_instance()->output;
		$CI->set_header("Content-disposition: filename=$filename;");
		$CI->set_header('Content-transfer-Encoding: binary');
		$CI->set_header('Last-modified: '.gmdate('D, d M Y H:i:s'));
		switch ($ext)
		{
			case "GIF"  :
				$CI->set_header("Content-type: image/gif");
				imagegif($this->temp_image);
				return $this;
				break;
			case "JPG" :
			case "JPEG" :
				$CI->set_header("Content-type: image/jpeg");
				imagejpeg($this->temp_image, NULL, $this->jpeg_quality);
				return $this;
				break;
			case "PNG" :
				$CI->set_header("Content-type: image/png");
