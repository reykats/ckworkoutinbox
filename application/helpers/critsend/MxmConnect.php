...
class MxmConnect {
	...
	*/
	...
	private static $user = 'system@workoutinbox.com';
    private static $key = '7evOQAJcCZUUoPY5V';
	...
	private static $hosts = array('http://mail1.messaging-master.com', 
	                        ...
	                        'http://mail29.messaging-master.com',
	...
	*/
	 ...
	 *
	 * @param tag is a string of less than 40 characters and without spaces or non-ASCII characters
	 * @return string containing the file; false otherwise
	 * @throw MxmException with soap error message
	 */
	public function getTag($tag) {
		...
		$auth = $this->getAuth();
		$res = $this->client->getTag($auth, $tag);
		echo $this->client->__getLastRequestHeaders() ;
		return $res;
	}
	
	public function getTagPaginated($tag, $start, $offset) {
		if (strlen($tag) > 40 || strpos($tag, ' ')) {
			throw new MxmException("invalid tag (connector answered)");
		}
		$auth = $this->getAuth();
		return $this->client->getTagPaginated($auth, $tag, $start, $offset);
	}	
